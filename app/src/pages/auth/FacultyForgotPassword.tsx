...
export function FacultyForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/auth/faculty/forget-password', {
        method: 'POST',
        ...
        <div className="w-full max-w-md bg-white border border-gray-200 shadow-md rounded-lg overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <p className="text-center text-sm text-gray-600 mt-4">
              Remembered? <Link to="/faculty/login" className="text-[#4a4a4a] font-semibold hover:underline cursor-pointer">Back to Login</Link>
            </p>
          </form>
        </div>
      </div>
